...
class GUI:
    ...
    def switch_handler(self, event):
        if self.use_simple_calculator:
            self._extracted_from_switch_handler_3(
                'Advanced Calculator', 'Switch to: Simple Calculator', False
            )
        else:
            self._extracted_from_switch_handler_3(
                'Simple Calculator', 'Switch to: Advanced Calculator', True
            )

    # TODO Rename this here and in `switch_handler`
    def _extracted_from_switch_handler_3(self, text, arg1, arg2):
        self.label.config(text=text)
        self.switch.config(text=arg1)
        self.use_simple_calculator = arg2
    ...
    def history_handler(self, event):
        ...
        for exp, out in history:
            self.display.insert(tk.END, f'Q: {str(exp)}' + '\n')
            self.display.insert(tk.END, f'A: {str(out)}' + '\n\n')
